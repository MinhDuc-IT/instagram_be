...
export class PostService {
  ...
  async getPosts(
    ...
    const mapped = await this.mapPostsToDto(posts, currentUserId);

    return this.buildPaginatedResponse(mapped, totalCount, page, limit);
  }
  ...
  async toggleLike(postId: string, userId: number) {
    ...
    if (existingLike) {
      ...
      // Tạo notification cho chủ post (chỉ khi like, không phải unlike)
      // Không tạo notification nếu user like chính post của mình
  ...
  async getHomeFeed(viewerId: number | null, page = 1, limit = 10) {
  ...
  async getSavedPosts(
    ...
    const [saved, totalCount] = await Promise.all([
